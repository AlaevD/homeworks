...
public final class FirstPartTasks {
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums.flatMap(album -> album.getTracks().stream().map(Track::getName))
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.map(album -> new Pair<>(album, album.getTracks()
                                                          .stream()
                                                          .mapToInt(Track::getRating)
                                                          .average()
                                                          .orElse(0)))
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream.reduce((a, b) -> ((a % modulo) * (b % modulo)) % modulo).orElse(0);
    }
    ...
    public static <R> Stream<R> filterIsInstance(Stream<?> s, Class<R> clazz) {
        return s.filter(clazz::isInstance).map(r -> (R) r);
    }
}